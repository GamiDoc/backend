...
func TestProjectPDFWithNotifyEmailRoute(t *testing.T) {
	tokenValue := authToken()
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/"+created.ID+"/generate-pdf", strings.NewReader(`{"notifyEmail":"user@example.com"}`))
...
func TestProjectPDFInvalidNotifyEmailRoute(t *testing.T) {
	tokenValue := authToken()
	...
	req := httptest.NewRequest(http.MethodPost, "/api/v1/projects/project-1/generate-pdf", strings.NewReader(`{"notifyEmail":"not-an-email"}`))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+tokenValue)
	rec := httptest.NewRecorder()
